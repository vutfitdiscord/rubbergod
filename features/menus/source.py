...
class DatabaseIteratorPageSource(PageSource):
    ...
    def __init__(self, query: Query, per_page=10):
        ...
        self.current_page = -1  # let's start from 0, see .get_page
    ...
    def is_paginating(self) -> bool:
        return self.current_page < self.get_max_pages()
...
class LeaderboardPageSource(DatabaseIteratorPageSource):
    ...
    def __init__(self, row_formatter: Union[str, Callable], base_embed: discord.Embed = None, *args, **kwargs):
        """
        Initialize this page source, ``query`` and ``row_formatter`` are required.


        row_formatter: :class:`str` or :class:`Callable`
            These are always supplied as keywords either into str.format or call of the formatter:
                - ``position`` :class:`int`
        ...
        base_embed: :class:`discord.Embed`, defaults to empty Embed
            ...
            it's ``.description`` will be used for pagination.
    ...
    async def _get_member_name(ctx: Context, member_id: Union[str, int]) -> str:
        ...
        return discord.utils.escape_markdown(member.name)

    async def _format_row(self, entry: Table, position: int, ctx: Context) -> str:
        """

        :raises :class:`AssertionError`:
            If neither ``member_id`` nor ``member_ID`` is found in entry's columns
        :returns: Formatted result of `self.row_format_str`.
        """
