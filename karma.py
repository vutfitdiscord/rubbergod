...
class Karma(BaseModel):
	...
	def valid_emoji(self, emojiId):
		row = BaseModel.get_row("bot_karma_emoji", "emoji_id = {}".format(emojiId))
	...
	def update_karma(self, member, emojiValue):
		...
		if self.get_karma(self, member.id):
	...
	def get_karma(self, user_id):
		row = BaseModel.get_row(self, "bot_karma", "member_id = {}".format(user_id))
		return row[1] if row else 0
