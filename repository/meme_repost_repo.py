...
class MemeRepostRepo:
    ...
    @staticmethod
    def create_repost(original_message_id:int, repost_message_id:int, author_id:int):
        item = MemeRepost(original_message_id=original_message_id, repost_message_id=repost_message_id, author_id=author_id)
        session.add(item)
        session.commit()