...
class FitWide(Base, commands.Cog):
    ...
    async def verify_check(self, inter: disnake.ApplicationCommandInteraction):
        ...
        guild = inter.guild
        verified = await self.helper.get_verified_members(guild)
    ...
    async def zapis_check(self, inter: disnake.ApplicationCommandInteraction):
        ...
        unmatching_members = await self.helper.get_members_with_unmatching_year(guild)
            ...
            for target_role, target_members in target_data.items():
                ...
                if ((
                    ...
                    message_prefix = (
                        ...
                        f"(protoze v API maji {target_year}): "
                    )
    ...
    async def role_check(self, inter: disnake.ApplicationCommandInteraction):
        ...
        for source_role, target_data in unmatching_members.items():
            for target_role, target_members in target_data.items():
                ...
                await inter.send(
                    f"Našel jsem {len(target_members)} lidi, kteří mají v API "
    ...
    async def update_db(
        ...
        for i, person in enumerate(all_persons):
            if i % 100 == 0:
                await inter.send(MessagesCZ.update_db_progress(count=i, total_count=persons_count))

            updated_person = await self.helper.check_api(login)
