...
async def autocomp_keys(inter: disnake.ApplicationCommandInteraction, user_input: str) -> List[str]:
    return [key for key in config.get_keys() if user_input.lower() in key][:25]
...
class DynamicConfig(Base, commands.Cog):
    ...
    @config.sub_command(description=Messages.config_set_brief)
    async def set_value(
        self,
    ...
    async def sync_template(self, inter: disnake.ApplicationCommandInteraction):
