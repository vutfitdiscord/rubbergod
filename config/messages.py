...
class Messages(metaclass=Formatable):
    ...
    upgraded_pocitani_caught_cheating = "Mazat a upravovat zprávy je zde zakázáno. Začínáme znovu: "
    ...
    # CONTESTVOTE
    contest_vote_calculate_contribution_brief = "Spočítá počet hlasů a jejich celkovou váhu pro daný příspěvek"
    contest_vote_top_contributions_brief = "Zobrazí prvních N počet nejvíce oblíbených příspěvků"
    ...
    contest_vote_file_too_big = "Soubor je příliš velký. Maximální velikost je 25MB."
    contest_vote_not_image = "Soubor není obrázek. Podporujeme pouze jpeg, jpg, png, gif."
    contest_vote_banned_user = "Byla ti odebrána možnost přídavat příspěvky do soutěže."
    contest_vote_submit_success = "Příspěvěk byl úspěšně přihlášen do soutěže."
    contest_vote_contribution = "### ID: {id}\n"
    contest_vote_max_contributions = "Dosáhl jsi maximálního počtu příspěvků. ({max_contributions})"
    contest_modal_title = "Odmitnutí příspěvku `ID {id}`"
    contest_modal_placeholder = "Důvod odmítnutí"
    contest_contribution_denied = "Příspěvek `ID {id}` byl odmítnut. Důvod:\n{reason}\n`@{author}`"
    contest_successful_deletion_no_reason = "Příspěvek byl odmítnut. Důvod nebyl odeslán uživateli. `@{author}`"
    contest_start_voting_brief = "Spustí se hlasování. Reakce budou přidány na příspěvky"
    contest_end_voting_brief = "Hlasování se ukončí. Reakce budou smazány a výsledky budou odeslány do contest kanálu"
...
```"""
    ...
    report_message_already_deleted = "### Nahlášená zpráva nebyla nalezena nebo byla smazána {author} `@{author_name}`"
    report_message_deleted = "### Nahlášená zpráva byla smazána uživatelem {author} `@{author_name}`"
    report_message_spam = "### Report #{id}, označen jako spam uživatelem {author} `@{author_name}`"
    report_message_not_spam = "### Report #{id}, označení jako spam bylo zrušeno uživatelem {author} `@{author_name}`"
